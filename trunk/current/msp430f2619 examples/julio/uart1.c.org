...
//
unsigned char TX_in_index, TX_out_index, TX_count;
unsigned char RX_in_index, RX_out_index, RX_count;
unsigned char TX_combuffer[TX_lenght];
unsigned char RX_combuffer[RX_lenght];

unsigned char evTXfull;
unsigned char evTXempty;
...
{
  if (TX_count == 0) evTXempty = 1;
  else
  {
    UCA0TXBUF = TX_combuffer[TX_out_index++];
    if (TX_out_index == TX_lenght) TX_out_index = 0;
    TX_count--;
    evTXfull = 0;
  }
}
...
{
  while (evTXfull);
  TX_combuffer[TX_in_index++] = c;
  if (TX_in_index == TX_lenght) TX_in_index = 0;
  if (++TX_count == TX_lenght) evTXfull = 1;
  if (evTXempty)
  {
    evTXempty = 0;
    IFG2 |= UCA0TXIFG;
