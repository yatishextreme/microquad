...
//void plota_barra(unsigned char linha, unsigned char leitura);
...
int main (void)
{
  ...
  cpu_config();
  lcd_init(cursorpiscante);
  config_serial(baud_115200, mod_115200); // uart (8,3b)
  i2c_config(NUNCHUCK, speed_100);//NUNCHUCK = 0xa4 , speed_100 = @16MHz --> 100KHz (Standard ou Slow Mode)
...
void i2c_change_slaveaddress(unsigned char slave_add)
{
  UCB0I2CSA = (slave_add >> 1); //desloca p/ direita
}
...
unsigned char nunchuck_getdata(void)
{
  ...
  //lcd_goto(0, 1); 
  /*
  printf("%02X\r", buffer[1]);
  printf("%02X\r", buffer[2]);
  printf("%02X\r", buffer[3]);
  printf("%02X\r", buffer[4]);
  printf("%02X\r", buffer[5]);
  printf("%02X\r", buffer[6]);
  */

  lcd_goto(6, 1);
  if(c == 1) printf("O"); else printf("X");
  lcd_goto(10, 1);
  if(z == 1) printf("O"); else printf("X");
