...
unsigned char nunchuck_getdata(void)
{
  ...
  plota_barra(2, buffer[1]);
  plota_barra(3, buffer[2]);
  plota_barra(4, buffer[3]);
  plota_barra(5, buffer[4]);
//  plota_barra(6, buffer[5]);

  lcd_goto(6, 7);
  if(c == 1) printf("O"); else 
  {
    printf("X");
    aonde = na_serial;
    printf("close\r\n");
...
void plota_barra(unsigned char linha, unsigned char leitura)
{
  ...
  if(valor > 42)
  {
    for(i=0; i<42; i++) lcd_wrdata(0x00);
    for(i=42; i<valor; i++) lcd_wrdata(0x3c);
    for(i=valor; i<84; i++) lcd_wrdata(0x00);
  } else
  {
    for(i=0; i<valor; i++) lcd_wrdata(0x00);
    for(i=valor; i<42; i++) lcd_wrdata(0x3c);
    for(i=42; i<84; i++) lcd_wrdata(0x00);
  }
}
